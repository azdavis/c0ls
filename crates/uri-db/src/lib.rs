...
impl UriDb {
  ...
  ///
  /// This only returns `None` if it is given a `UriId` generated by a different
  /// `UriDb`, which probably isn't a good idea.
  pub fn get(&self, file_id: UriId) -> Option<&Uri> {
    self.id_to_uri.get(file_id.0 as usize)
  }
